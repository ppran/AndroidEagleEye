...
public class CipherHook extends MethodHook {
	...
	private enum Methods {
		doFinal,getProvider,getIV,getAlgorithm,wrap,unwrap,getMaxAllowedKeyLength,
		getInstance, init, update, getBlockSize, getMaxAllowedParameterSpec, getParameters,
		getExemptionMechanism, updateAAD, getOutputSize
	...
	private void logMsg(ArrayList<byte[]> plainByteList, ArrayList<byte[]> encryptByteList,
		...
		for(int i=0; i<minSize; i++){
			...
			String msg = String.format("$$$\"BasicCipherSpecial\":[\"%d\",\"%d\",\"false\"]$$$\"InvokeApi\":\"%s;->%s\"$$$\"CryptoUsage\":{\"plaintext\":" +
					"\"%s\",\"encrypttext\":\"%s\"$$$\"operation\":\"%s\"$$$\"CipherAlgorithm\":\"%s\"$$$\"id\":\"%d\" }}",
					uid, Util.FRAMEWORK_HOOK_SYSTEM_API, this.getClassName(), this.getMethodName(),plainText, encryptText, operation, algorithm, id);
